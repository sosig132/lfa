...
tape_copy = [blank for i in range(len(word_to_test)+20000)]
...
def accepts(symbols,word_to_test, transition_dict, reject, accept, start):
    ...
    while True:
        ...
        print(head)
        state_prev=state
        if tape[head] not in transition_dict[state].keys():
            return 'Rejected'
        ...
        if transition_dict[state_prev][tape[head]][len(transition_dict[state_prev][tape[head]])-1]=='R':
            head+=1
            right=1
        elif transition_dict[state_prev][tape[head]][len(transition_dict[state_prev][tape[head]])-1]=='L':

            head-=1

        elif transition_dict[state_prev][tape[head]][len(transition_dict[state_prev][tape[head]])-1]=='L' and tape[head-1]==blank:
            if len(transition_dict[state_prev][tape[head]])==3:
                tape[head]=transition_dict[state_prev][tape[head]][1]
                tape_copy[head]=transition_dict[state_prev][tape_copy[head]][1]
                if tape!=tape_copy:
                    return 'EROARE: TAPEURI DIFERITE'
            continue
        if len(transition_dict[state_prev][tape[head-right]])==3:
            tape[head-right]=transition_dict[state_prev][tape[head-right]][1]
            print(tape_copy[head-right])
            print(tape[head-right])
            tape_copy[head-right]=transition_dict[state_prev][tape_copy[head-right]][1]
            if tape!=tape_copy:
                return 'EROARE: TAPEURI DIFERITE'

v = accepts(tape_symbols, word_to_test, transition_dict, reject, accept, start)

if tape == tape_copy and v == 'Accepted':
    print('Accepted')
elif v=='Accepted':
    print('Tapes not equal')