...
for transition in transitions:
    ...
    if transition_list[0] not in states:
        print(f'Invalid NFA input file: Invalid state(There is no state "{transition_list[0]}" in States)')
        valid=0
        break
    if transition_list[2] not in states:
        print(f'Invalid NFA input file: Invalid state(There is no state "{transition_list[2]}" in States)')
        valid=0
        break
    if transition_list[1] not in letters:
        print(f'Invalid NFA input file: Invalid letter(There is no letter "{transition_list[1]}" in Sigma)')
        valid=0
        break
...
#            print(f'Infalid DFA input file: Test for determinism fail')
#            valid=0
#    else:
